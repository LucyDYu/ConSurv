...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # Execute normalization and convert data type
        output = self._norm(x.float()).type_as(x)
        # Apply learnable weights and return
...
class BottleneckTransFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # Process through the second attention layer
        ...
        # Process through the first attention layer
        x = self.attn1(x)
...
class DropX2Fusion(nn.Module):
    ...
    def forward(self, x1, x2):
        # Directly return x1, ignoring x2
        return x1
...
def DiffSoftmax(logits, tau=1.0, hard=False, dim=-1):
    ...
    if hard:
        # If using hard mode, apply straight-through estimation
...
class RoutingNetwork(nn.Module):
    ...
    def forward(self, x1, x2, temp=1.0, hard=False):
        # Process input through two FC layers
        x1, x2 = self.fc1(x1), self.fc2(x2)
        # Compute average and merge
...
class MoME(nn.Module):
    ...
    def forward(self, x1, x2, hard=False, **kwargs):
        ...
        if hard:
            # If hard mode, select branch with highest probability
