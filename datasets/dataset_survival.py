...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path = 'dataset_csv/ccrcc_clean.csv', mode = 'omic', apply_sig = False, 
        ...
        missing_slides_ls = ['TCGA-A7-A6VX-01Z-00-DX2.9EE94B59-6A2C-4507-AA4F-DC6402F2B74F.svs',
                             ...
                             'TCGA-G2-A2EJ-01Z-00-DX1.BBADB2AF-BF32-4678-A54B-F82CA11DC715.svs',
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str='train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(df_slice, metadata=self.metadata, mode=self.mode, 
                                  ...
                                  label_col=self.label_col, patient_dict=self.patient_dict, num_classes=self.num_classes, n_bins=self.n_bins, task_id=self.task_id)
        else:
            split = None
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir, mode: str='omic', n_bins: int=4, task_id: int=0, **kwargs):
        """
        Initialize survival analysis dataset
        
        Args:
            data_dir: Data directory
            mode: Data mode, default is 'omic'
            n_bins: Number of bins per task, default is 4
            task_id: Current task ID, default is 0
        ...
        self.label_offset = task_id * n_bins
    ...
    def __getitem__(self, idx):
        ...
        # Update label, temporarily disabled
        ...
        if not self.use_h5:
            if self.data_dir:
                ...
                # Get path and omic separate features
                # Try to match the Mammoth format
                    ...
                    # Accessing genomic features by column names and converting to tensors
                    # omic1 = torch.tensor(self.genomic_features[self.omic_names[0]].values[idx])
                    # omic2 = torch.tensor(self.genomic_features[self.omic_names[1]].values[idx])
                    # omic3 = torch.tensor(self.genomic_features[self.omic_names[2]].values[idx])
                    # omic4 = torch.tensor(self.genomic_features[self.omic_names[3]].values[idx])
                    # omic5 = torch.tensor(self.genomic_features[self.omic_names[4]].values[idx])
                    # omic6 = torch.tensor(self.genomic_features[self.omic_names[5]].values[idx])
                    ...
                    #     print("contain logits")
                    #     return (path_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, self.logits[idx])
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2, n_bins=4, task_id=0):
        self.use_h5 = False
        self.slide_data = slide_data
        ...
        self.n_bins = n_bins
        self.task_id = task_id
        self.label_offset = task_id * n_bins  # Compute label offset
