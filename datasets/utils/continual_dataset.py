...
class MammothDatasetWrapper(Dataset, object):
    ...
    def extend_return_items(self, ret_tuple: Tuple[torch.Tensor, int, torch.Tensor, Optional[torch.Tensor]],
        ...
        for name in self.extra_return_fields:
            ...
            # Ensure `data` exists and its length matches `attr`
            if hasattr(self, 'data') and len(attr) == len(self.data):
                c_idx = index
            else:
                c_idx = self.indexes[index]
            # print(f'c_idx {c_idx}')
            # # print(f'attr {attr}')
            # print(f'attr[c_idx] {attr[c_idx]}')
            tmp_tuple.append(attr[c_idx])
...
class ContinualDataset(object):
    ...
    @staticmethod
    def evaluate(model: 'ContinualModel', dataset: 'ContinualDataset', last=False, return_loss=False, **kwargs):
        """
        ...
        check_fn_dynamic_type(dataset.eval_fn, EvalFn, strict=False)
        return dataset.eval_fn(model, dataset, last=last, return_loss=return_loss, **kwargs)
