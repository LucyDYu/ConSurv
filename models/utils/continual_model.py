...
from utils.training_utils import wait_for_memory
...
class ContinualModel(nn.Module):
    ...
    def __init__(self, backbone: 'MammothBackbone', loss: nn.Module,
        ...
        try:
            ...
            logging.error(f"could not initialize kornia transforms. Error: {e}")
    ...
    def load_buffer(self, buffer):
        ...
        if isinstance(buffer, Buffer):
            ...
            assert self.buffer.buffer_size == len(buffer['examples']), "Buffer size mismatch. Expected {} got {}".format(
                self.buffer.buffer_size, len(buffer['examples']))
            for k, v in buffer.items():
                setattr(self.buffer, k, v)
            self.buffer.attributes = list(buffer.keys())
            self.buffer.num_seen_examples = len(buffer['examples'])
        else:
    ...
    def begin_task(self, dataset: 'ContinualDataset', **kwargs) -> None:
        """
    ...
    def end_task(self, dataset: 'ContinualDataset', **kwargs) -> None:
        """
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Args:
            ...
            **kwargs: additional inputs for multi-modal models (e.g., x_path, x_omic1, etc.)

        Returns:
            the result of the computation
        """
        if kwargs:
            # For multi-modal inputs like WSI data
    ...
    def meta_observe(self, *args, **kwargs):
        ...
        Returns:
            the value of the loss function, or a dict containing loss and other information
        """
        if not wait_for_memory(min_memory_gb=20, max_retries=None, wait_interval=5):
            logging.info("Memory is small. Process may fail due to oom.")
        ...
        if 'wandb' in sys.modules and not self.args.nowand:
            ...
            if isinstance(ret, dict):
                ...
                # ret = ret['loss'] # Return the whole dict
        ...
        return ret # Return the whole dict
    ...
    def meta_end_task(self, dataset, **kwargs):
        """
    ...
    def autolog_wandb(self, locals, extra=None):
        ...
        """
        try:
            # Only log every N steps, where N is determined by args.log_freq (defaulting to 100)
            log_freq = getattr(self.args, 'log_freq', 100)
            if (self.task_iteration % log_freq == 0) and not self.args.nowand and not self.args.debug_mode:
                ...
                wandb.log(tmp, commit=False)
        except Exception as e:
            try:
                # Print full error traceback (with line numbers)
                error_msg = f"WandB logging failed: {str(e)}\n{traceback.format_exc()}"
                logging.error(error_msg)
                print(f"\033[31mERROR: {error_msg}\033[0m")  # Red highlighted output
                print(tmp)
            except Exception as e:
                print(f"\033[31mERROR: {e}\033[0m")  # Red highlighted output
