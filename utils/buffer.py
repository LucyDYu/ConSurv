...
class Buffer:
    ...
    def __init__(self, buffer_size: int, device="cpu", sample_selection_strategy='reservoir', **kwargs):
        ...
        self.attributes = ['examples', 'labels', 'logits', 'task_labels', 'true_labels']  # Define buffer attributes
        ...
        assert sample_selection_strategy.lower() in ['reservoir', 'lars', 'labrs', 'abs', 'unlimited'], f"Invalid sample selection strategy: {sample_selection_strategy}"  # Ensure valid selection strategy

        if sample_selection_strategy.lower() == 'abs':  # If strategy is abs
            assert 'dataset' in kwargs, "The dataset is required for ABS sample selection"  # Ensure dataset is provided
            self.sample_selection_fn = ABSSampling(buffer_size, device, kwargs['dataset'])  # Initialize ABS sampling
        elif sample_selection_strategy.lower() == 'lars':  # If strategy is lars
            self.sample_selection_fn = LARSSampling(buffer_size, device)  # Initialize LARS sampling
        elif sample_selection_strategy.lower() == 'labrs':  # If strategy is labrs
            self.sample_selection_fn = LossAwareBalancedSampling(buffer_size, device)  # Initialize LABRS sampling
        elif sample_selection_strategy.lower() == 'unlimited':  # If strategy is unlimited
            self.sample_selection_fn = lambda x: x  # Set unlimited sampling
            self._buffer_size = 10  # Initial buffer size, will expand as needed
        else:  # Default strategy: reservoir
    ...
    def serialize(self, out_device='cpu'):
        ...
        """
        return {attr_str: getattr(self, attr_str).to(out_device) for attr_str in self.attributes if hasattr(self, attr_str)}  # Return serialized attributes dict
    ...
    def __len__(self):
        ...
        """
        if self.sample_selection_strategy == 'unlimited':  # If strategy is unlimited
            return self.num_seen_examples  # Return seen examples count
        return min(self.num_seen_examples, self.buffer_size)  # Return min of seen examples and buffer size
    ...
    def init_tensors(self, examples: torch.Tensor, labels: torch.Tensor,
        ...
        """
        for attr_str in self.attributes:  # Iterate over all attributes
            attr = eval(attr_str)  # Get attribute
            if attr is not None and not hasattr(self, attr_str):  # If attribute is not None and does not exist in buffer
                typ = torch.int64 if attr_str.endswith('els') else torch.float32  # Set data type based on attribute type
                setattr(self, attr_str, torch.zeros((self._buffer_size,  # Create tensor
                        *attr.shape[1:]), dtype=typ, device=self.device))  # Initialize tensor
            elif hasattr(self, attr_str):  # If tensor already exists
                if self.num_seen_examples < self._buffer_size:  # If buffer is not full
                    old_tensor = getattr(self, attr_str)  # Get old tensor
                    pad = torch.zeros((self._buffer_size - old_tensor.shape[0], *attr.shape[1:]), dtype=old_tensor.dtype, device=self.device)  # Create padding tensor
                    setattr(self, attr_str, torch.cat([old_tensor, pad], dim=0))  # Concatenate old tensor and padding
    ...
    def buffer_size(self):
        ...
        """
        if self.sample_selection_strategy == 'unlimited':  # If strategy is unlimited
            return int(1e9)  # Return max integer
    ...
    def buffer_size(self, value):
        ...
        """
        if self.sample_selection_strategy != 'unlimited':  # If strategy is not unlimited
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None, true_labels=None, sample_selection_scores=None):
        ...
        """
        if not hasattr(self, 'examples'):  # If examples attribute does not exist in buffer
        ...
        for i in range(examples.shape[0]):  # Iterate over each example
            if self.sample_selection_strategy == 'abs' or self.sample_selection_strategy == 'labrs':  # If strategy is abs or labrs
            ...
            if index >= 0:  # If index is valid
                if self.sample_selection_strategy == 'unlimited' and self.num_seen_examples > self._buffer_size:  # If unlimited strategy and seen examples exceed buffer size
                ...
                if task_labels is not None:  # If task labels is not None
                    self.task_labels[index] = task_labels[i].to(self.device)  # Add task label to buffer
                if attention_maps is not None:  # If attention maps is not None
                    self.attention_maps[index] = [at[i].byte().to(self.device) for at in attention_maps]  # Add attention map to buffer
                if sample_selection_scores is not None:  # If sample selection scores is not None
                    self.sample_selection_fn.update(index, sample_selection_scores[i])  # Update sample selection scores
                if true_labels is not None:  # If true labels is not None
                    self.true_labels[index] = true_labels[i].to(self.device)  # Add true label to buffer
    ...
    def get_data(self, size: int, transform: nn.Module = None, return_index=False, device=None,
        ...
        if mask_task_out is not None:  # If task masking is needed
            ...
            assert hasattr(self, 'task_labels') or hasattr(self, 'labels')  # Ensure task labels or labels exist
            samples_mask = (self.task_labels != mask_task_out) if hasattr(self, 'task_labels') else self.labels // cpt != mask_task_out  # Create sample mask

        num_avail_samples = self.examples.shape[0] if mask_task_out is None else samples_mask.sum().item()  # Get available sample count
        num_avail_samples = min(self.num_seen_examples, num_avail_samples)  # Take min of seen examples and available samples

        if size > min(num_avail_samples, self.examples.shape[0]):  # If requested size exceeds available samples
            size = min(num_avail_samples, self.examples.shape[0])  # Set size to available sample count
        ...
        if transform is None:  # If no transform
            def transform(x): return x  # Define identity transform

        selected_samples = self.examples[choice] if mask_task_out is None else self.examples[samples_mask][choice]  # Get selected samples

        if return_not_aug:  # If returning non-augmented samples
            if not_aug_transform is None:  # If no non-augmented transform
                def not_aug_transform(x): return x  # Define identity transform
        ...
        for attr_str in self.attributes[1:]:  # Iterate over all attributes
            if hasattr(self, attr_str):  # If attribute exists
                ...
                selected_attr = attr[choice] if mask_task_out is None else attr[samples_mask][choice]  # Get selected attribute
                ret_tuple += (selected_attr.to(target_device),)  # Add selected attribute to return tuple
...
def fill_buffer(buffer: Buffer, dataset: 'ContinualDataset', t_idx: int, net: 'MammothBackbone' = None, use_herding=False,
    ...
    if net is not None:  # If model is provided
        ...
        assert not use_herding, "Herding strategy requires a model instance"  # Herding strategy requires a model instance
    ...
    if norm_trans is None:  # If no normalization transform
        def norm_trans(x): return x  # Define identity transform

    if 'logits' in buffer.used_attributes:  # If buffer uses logits
    ...
    for data in loader:  # Iterate over data loader
        ...
        if not x.size(0):  # If data is empty
            continue  # Skip
        a_x.append(not_norm_x.cpu())  # Add unnormalized data
    ...
    for _y in a_y.unique():  # Iterate over unique labels
        ...
        if use_herding:  # If using herding strategy
            ...
            while i < samples_per_class and i < feats.shape[0]:  # Loop until reaching sample count or feature count
                ...
                feats[idx_min] = feats[idx_min] + 1e6  # Mark used feature
    ...
    assert len(buffer.examples) <= buffer.buffer_size, f"buffer overflowed its maximum size: {len(buffer)} > {buffer.buffer_size}"  # Ensure buffer did not overflow
    assert buffer.num_seen_examples <= buffer.buffer_size, f"buffer has been overfilled, there is probably an error: {buffer.num_seen_examples} > {buffer.buffer_size}"  # Ensure seen examples did not overflow
