...
def mask_classes(outputs: torch.Tensor, dataset: 'ContinualDataset', k: int) -> None:
    ...
    outputs[:, :start_c] = -float('inf')  # Mask out previous task classes
    outputs[:, end_c:num_classes] = -float('inf')  # Mask out future task classes
...
def evaluate(model: 'ContinualModel', dataset: 'ContinualDataset', last=False, return_loss=False) -> Tuple[list, list]:
    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        # Check for potential bugs in accuracy calculation
