...
def collate_MIL_survival_sig(batch):
    ...
    c = torch.FloatTensor([item[9] for item in batch])
     # Check if extra elements exist
    result = [img, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c]
    if all(len(item) > 10 for item in batch):
        num_elements = len(batch[0])
        # Iterate over additional element indices
        for i in range(10, num_elements):
            elements = [item[i] for item in batch]
            # print(elements)
            # Type checking
            if all(isinstance(element, torch.Tensor) for element in elements):
                new_element = torch.cat(elements, dim=0)
            elif all(isinstance(element, (int, np.integer)) for element in elements):
                new_element = torch.LongTensor(elements)  # Integer type
            elif all(isinstance(element, (float, np.floating)) for element in elements):
                new_element = torch.FloatTensor(elements)  # Float type
            else:
                new_element = torch.tensor(elements)  # Auto-inferred type

            result.append(new_element)

    return result

def select_collate(mode):
    if mode == 'coattn':
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1, joint_flag=False, args=None):
    """
        return either the validation loader or training loader 
    """
    collate = select_collate(mode)
    num_workers = args.num_workers if args is not None and hasattr(args, 'num_workers') else 4
    ...
    if not testing:
        if training:
            if weighted:
                if joint_flag:
                    weights = make_weights_for_balanced_classes_split_joint(split_dataset)
                else:    
...
def make_weights_for_balanced_classes_split(dataset):
    """
    Calculate weights for balanced class sampling in data loader
    
    Args:
        dataset: Dataset object containing slide_cls_ids and getlabel method
        
    Returns:
        torch.DoubleTensor: Weight tensor for balanced sampling, where each sample's weight
                           is inversely proportional to its class frequency
    """
    # Get total number of samples
    N = float(len(dataset))                                           
    
    # Calculate weight per class as total_samples/samples_in_class
...
def make_weights_for_balanced_classes_split_joint(joint_dataset):
    """
    Calculate weights for balanced class sampling in data loader
    
    Args:
        joint_dataset: Dataset object containing slide_cls_ids and getlabel method
    ...
    """
    # Get total number of samples
    N = float(len(joint_dataset))                                           
    
    # Calculate weight per class as total_samples/samples_in_class. inverse of class frequency
    weight_per_class_list = []
    for dataset in joint_dataset.datasets:
        weight_per_class = [N/len(dataset.slide_cls_ids[c]) for c in range(len(dataset.slide_cls_ids))]                                                                                                     
        weight_per_class_list.append(weight_per_class)
    
    # Initialize weight array
    weight = [0] * int(N)                                           
    
    # Assign weight to each sample based on its class
    for idx in range(len(joint_dataset)):   
        y = joint_dataset.getlabel(idx)                        
        dataset_idx = joint_dataset.get_dataset_idx(idx)
        weight[idx] = weight_per_class_list[dataset_idx][y]                                  
...
def nll_loss(hazards, S, Y, c, alpha=0.4, eps=1e-7):
    ...
    Args:
        hazards: Hazard predictions
        S: Survival function (can be None, will be calculated from hazards)
        Y: Time labels
        c: Censorship indicators
        alpha: Weight parameter for uncensored loss
...
def get_custom_exp_code(args):
    ...
    dataset_path_origin = 'dataset_csv'
    dataset_path = dataset_path_origin + '_padding'
    param_code = ''
    ...
    if args.apply_sigfeats:
      param_code += '_sig'
      dataset_path = dataset_path_origin + '_sig' + '_padding'
    ...
    param_code += '_bs_micro' + str(args.bs_micro)

    if args.n_bottlenecks:
        param_code += '_n_bottleneck' + str(args.n_bottlenecks)
