...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    for epoch in range(args.start_epoch,args.max_epochs):
        if args.mode == 'coattn':
            ...
            elif args.model_type == 'mome' or args.model_type == 'motcat':
...
def init_model(args: Namespace):
    ...
    if args.model_type == 'snn':
        ...
        from models.model_motcat import MOTCAT_Surv
        model_dict = {'ot_reg': args.ot_reg, 'ot_tau': args.ot_tau, 'ot_impl': args.ot_impl, 'fusion': args.fusion,
    ...
    if args.load_model:
        model.load_state_dict(torch.load(args.path_load_model, weights_only=True))
    print('Done!')
    return model


def train_CL(datasets: tuple, cur: int, args: Namespace, tcga_dataset_name: Optional[str] = None):
    """
        train for a single fold
    """
    print('\nTraining Fold {}!'.format(cur))
    args.writer_dir = os.path.join(args.results_dir, tcga_dataset_name, str(cur))
    if not os.path.isdir(args.writer_dir):
        os.mkdir(args.writer_dir)
    if args.log_data:
        from tensorboardX import SummaryWriter
    ...
    print('\nInit train/val/test splits...', end=' ')
    train_split, val_split = datasets
    save_splits(datasets, ['train', 'val'], os.path.join(args.results_dir, tcga_dataset_name, 'splits_{}.csv'.format(cur)))
    print('Done!')
    print("Training on {} samples".format(len(train_split)))
    print("Validating on {} samples".format(len(val_split)))
    ...
    if args.load_model:
        print('Loading model from {}'.format(args.path_load_model))
        model.load_state_dict(torch.load(args.path_load_model, weights_only=True))
    else:
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        if args.mode == 'coattn':
            if args.model_type == 'mcat':
                ...
                train_loop_survival_coattn(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn,
                                           reg_fn, args.lambda_reg, args.gc, args)
                val_latest, c_index_val, stop = validate_survival_coattn(cur, epoch, model, val_loader, args.n_classes,
                                                                         early_stopping, monitor_cindex, writer,
                                                                         loss_fn, reg_fn, args.lambda_reg,
                                                                         args.results_dir, args)
            elif args.model_type == 'mome' or args.model_type == 'motcat':
                from trainer.mb_trainer import train_loop_survival_coattn_mb, validate_survival_coattn_mb
                train_loop_survival_coattn_mb(epoch, args.bs_micro, model, train_loader, optimizer, args.n_classes,
                                              writer, loss_fn, reg_fn, args.lambda_reg, args.gc, args)
                val_latest, c_index_val, stop = validate_survival_coattn_mb(cur, epoch, args.bs_micro, model,
                                                                            val_loader, args.n_classes, early_stopping,
                                                                            monitor_cindex, writer, loss_fn, reg_fn,
                                                                            args.lambda_reg, args.results_dir, args)
            else:
                raise NotImplementedError
        else:
            from trainer.mil_trainer import train_loop_survival, validate_survival
            train_loop_survival(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn, reg_fn,
                                args.lambda_reg, args.gc, args)
            val_latest, c_index_val, stop = validate_survival(cur, epoch, model, val_loader, args.n_classes,
        ...
        if c_index_val > max_c_index:
            ...
            epoch_max_c_index = epoch
            save_name = 's_{}_checkpoint'.format(cur)
            if tcga_dataset_name is not None:
                save_name = tcga_dataset_name + '_' + save_name
            ...
            torch.save(model.state_dict(), os.path.join(
                args.results_dir, tcga_dataset_name, save_name + ".pt".format(cur)))
            best_val_dict = val_latest
