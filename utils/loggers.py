...
class Logger:
    ...
    def get_target_path(self, file_name):
        ...
        path = smart_joint(target_folder, self.setting, self.dataset, self.model, file_name)
        return path
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        path = self.get_target_path("logs.pyd")
        ...
        if isinstance(mean_acc, float):  # domain or gcl
            ...
            d2 = {f'{prefix}_class_mean_accs{postfix}': mean_acc[0], f'{prefix}_task_mean_accs{postfix}': mean_acc[1],
                  ...
                  **{f'{prefix}_task_acc_{i}{postfix}': a for i, a in enumerate(accs[1])},
                  'Task': t,
                  # X-axis shows epoch data
                  f'{prefix}_class_mean_performance': mean_acc[0], f'{prefix}_task_mean_performance': mean_acc[1],
                  **{f'{prefix}_class_performance_{i}': a for i, a in enumerate(accs[0])},
                  **{f'{prefix}_task_performance_{i}': a for i, a in enumerate(accs[1])}}
