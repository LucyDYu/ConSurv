...
@torch.no_grad()
def evaluate_wsi_joint(model: 'ContinualModel', dataset: 'ContinualDataset', last=False, return_loss=False, **kwargs) -> Tuple[list, list]:
    """
    ...
    """
    logging.info(f"Validation iteration on joint training setting")
    ...
    for k, task_name in enumerate(args.task_name_order):
        
        logging.info(f"Validation task {k} {task_name}. Joint training, evaluate for each task separately, metrics take average")
            
        # Initialize metrics for current task
        correct, correct_mask_classes, total = 0.0, 0.0, 0.0
        test_loader = dataset.test_loaders[0] # only one test loader for joint training
        test_iter = iter(test_loader)
        i = 0
        # initialize survival metrics
        test_concat_dataset = test_loader.dataset.dataset
        test_task_dataset = test_concat_dataset.datasets[k]
        length_test_loader = len(test_task_dataset) # evaluate for each task datasets separately, since they have different time coordinate
        val_loss_surv, val_loss, all_risk_scores, all_censorships, all_event_times = initialize_survival_metrics(length_test_loader, args)

        times_coordinate = np.append(0, dataset.datasets[args.task_name_order[k]].bins[1:])

        kwargs['times_coordinate'] = times_coordinate
        logging.info(f"times_coordinate for {task_name}: {times_coordinate}")

        # Get slide_ids
        slide_ids = test_task_dataset.slide_data['slide_id']
        ...
        while True:
            ...
            # skip data from other tasks
            if data_task_label != k:
                continue 
        ...
        val_task_dataset = test_task_dataset
        train_concat_dataset = dataset.train_loader.dataset.dataset
        train_task_dataset = train_concat_dataset.datasets[k]
        all_data_survival = _extract_survival_metadata(train_task_dataset, val_task_dataset)
        ...
        assert c_index == c_index_old, f"Calculated c_index {c_index} differs from previous calculation {c_index_old}"

        print(
            'Epoch:{} Val c-index: {:.4f} | Final Val c-index2: {:.4f}'.format(
                epoch,
                c_index,
                val_cindex_ipcw,
            ))
    ...
    accs = [np.mean(accs)] 
    accs_mask_classes = [np.mean(accs_mask_classes)]
    mean_metric_dict_tuple = tuple()
    for metric_dict in metric_dict_tuple:
        mean_metric_dict = {f'val_{args.joint_task_name}': np.mean(list(metric_dict.values()))}
        mean_metric_dict_tuple += (mean_metric_dict,)

    return accs, accs_mask_classes, val_latest_dict, mean_metric_dict_tuple