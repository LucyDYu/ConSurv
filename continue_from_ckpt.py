...
###########################################################
# Cell 4: Setup command line arguments
###########################################################
...
# Force CUDA to be invisible, disable GPU
...
model_config='default' # Rehearsal uses default, since buffer size can be adjusted
buffer_size=32 # total 16 classes
# buffer_size=64
# start from task 2
fold=4
start_from=2
checkpoint_name="<your-checkpoint-name>"  # e.g., "consurv_seq-survival_None_32_20_YYYYMMDD-HHMMSS_xxxxxxxx_1"

checkpoint_path=f'checkpoints/{checkpoint_name}.pt'


cmd_args_change=[
    f'--fold={fold}', # setup fold
    f'--model={model_name}',
...
cmd_args_default=[
    ...
    '--num_workers=24',
    # '--num_workers=0' # For debugging convenience; not needed for formal training
]
...
# Set new sys.argv to simulate command line input
if in_ipynb():
    print("in ipynb")
...
if __name__ == '__main__':
    # Log the start of Mammoth! on the current host; this helps to identify logging output from different runs
...
# Create results_dir directory if it doesn't exist.
